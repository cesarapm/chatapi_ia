...
const knowledgeBase = {
  ...
  financing_options:
    "Yes, we have programs for both. With land, you can do 0 down payment. Without land, a down payment is required.",
  permits: "Yes, we take care of the permits.",
  itin: "Yes, we have lenders that accept ITIN or Social.",
  own_land:
    "If you own the land, the next step is to submit an application so we can determine your budget. Then, you can choose models within that price range.",
  process_time:
    "Depending on the permits, the process can take from 4 to 6 months, or even faster depending on the county.",
  customization:
    "Yes, we have architects ready to modify, add, or design your dream home.",
  interior_finish:
    "Yes, 100% of our homes come with crown molding and wood trim.",
  full_support:
    "Yes, we assist with financing, delivery, permits, setup, excavation, concrete, and finishes.",
...
function containsExplicitQuestion(message) {
  ...
  return (
    msg.includes("?") ||
    questionWords.some(
      (word) => msg.startsWith(word) || msg.includes(" " + word + " ")
    )
  );
}
router.post("/", async (req, res) => {
  ...
  if (isQuestion) {
    for (const category in keywords) {
      if (
        keywords[category].some((keyword) => messageLowerCase.includes(keyword))
      ) {
  ...
  if (foundAnswer) {
    ...
    return res.json({
      reply: `${foundAnswer} Would you like an advisor to contact you?`,
    });
  }
  ...
  if (userData.etapa === "inicio") {
    ...
    return res.json({
      reply:
        "Sorry, only an advisor can give you that information. Would you like an advisor to contact you?",
    });
  }
  ...
  if (userData.etapa === "esperando_confirmacion") {
    if (respuestasAfirmativas.some((word) => messageLowerCase.includes(word))) {
      ...
      return res.json({
        reply:
          "Great, to connect you with an advisor, I need some information. Let's start with your name.",
      });
    } else if (
      respuestasNegativas.some((word) => messageLowerCase.includes(word))
    ) {
      delete conversations[userId];
      conversations[userId] = { etapa: "inicio" };
      return res.json({
        reply:
          "Understood, if you need help another time, I'll be here. Thanks!",
      });
    }
  }
  ...
  switch (userData.etapa) {
    case "explicacion_datos":
      ...
      conversations[userId] = userData;
      return res.json({
        reply: `Nice to meet you, ${message}. What's your email address?`,
      });

    case "pedir_email":
      if (!message.includes("@") || !message.includes(".")) {
        return res.json({
          reply: "The email address you entered is invalid. Please try again.",
        });
      }
      ...
      conversations[userId] = userData;
      return res.json({
        reply: "Perfect. Now, could you give me your phone number?",
      });

    case "pedir_telefono":
      if (!/^\d{10}$/.test(message)) {
        return res.json({
          reply: "The phone number must contain exactly 10 numeric digits.",
        });
      }
      ...
      conversations[userId] = userData;
      return res.json({
        reply: "Thank you. Finally, what is your message or question?",
      });
    ...
    case "pedir_mensaje":
      ...
      contentSid: process.env.TWILIO_CONTENT_SID,
        ...
        return res.json({
          reply:
            "Thank you! Your message has been sent. An advisor will contact you shortly.",
        });
      } catch (error) {
        console.log(error);
